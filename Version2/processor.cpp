...
class Bus {
    private: 
                ...
                if (currentInstruction == 'R') {
                    switch (cacheState) {
                        ...
                        case CacheState::INVALID:
                            ...
                            if (owners.top() == core && state == BusState::IDLE) {
                    ...
                    switch (cacheState) {
                        case CacheState::MODIFIED:
                        case CacheState::EXCLUSIVE:
                            CacheState cacheState = getWriteCacheState(core, currentAddress);
                            if (owners.top() == core) owners.pop();
                            cores[core].line++;
                            break;
                        case CacheState::SHARED:
        ...
        CacheState getInvalidCacheState(int core, unsigned int currentAddress) {
            ...
            for (int i = 0; i < E; i++) {
                if (cores[core].cacheLRU[setIndex][i] < cores[core].cacheLRU[setIndex][min]) min = i;
            }
        ...
        void runSnoop(int core) {
            switch (state) {
                case BusState::INVALID:
                    ...
                    break;
                case BusState::RM:
                    if (owners.top() <= 3 && containsAddress(core, address)) {
                        owners.push(4 + core);
                        state = BusState::TRANSACTION;
                        source = core;
                    ...
                    break;
                case BusState::RWITM:
                    if (owners.top() <= 3 && getCacheState(core, address) == CacheState::MODIFIED) {
                        owners.push(4 + core);
                        state = BusState::TRANSACTION;
                        source = core;
                        destination = 4;
            ...
            switch (state) {
                ...
                case BusState::TRANSACTION:
                    if (remainingCycles == 0) {
                        if (destination < 4) addToCache(destination);
                        if (owners.top() >= 4 && owners.top() <= 7) owners.pop();
                        state = BusState::INVALID;
                    }
                    break;
                default:
        ...
        bool runCycle () {
            ...
            int tempOwner = owners.top();

            runCore(tempOwner);

            for (int i = 0; i < 3; i ++) {
                if (i != tempOwner) runCore(i);
            }
    ...
    public:
        ...
        state(BusState::IDLE), address(0), s(s), E(E), b(b) {
